...
FIELDER_SPEED = 1.5; FIELDER_MAX_MOVE_FRAMES = FPS // 2
CROWD_AREA_HEIGHT = SCREEN_HEIGHT * 0.20
CROWD_FAN_COLORS = [(200,50,50), (50,50,200), (50,150,50), (180,180,30), (100,100,100), (220,120,30)]
...
class Scoreboard:
    def __init__(self, font, screen_width):
        ...
        self.max_overs = 3 # Default to 3 for animation (20 balls)
        self.last_ball_outcome_display = "-"
        self.target = 0

        self.text_color = BLACK
        self.bg_color = (220, 220, 220, 200) # Light grey, slightly more opaque
        self.padding = 10 # Increased padding
        self.line_height = font.get_linesize() + 4 # Increased line height
        self.y_position = 45 # Position below team names overlay

    def set_teams(self, team_a_name, team_b_name, current_batting_team_name):
        self.team_a_name = team_a_name
        self.team_b_name = team_b_name
        self.batting_team_name = current_batting_team_name
        # Calculate max_overs based on typical 20-ball log for animation
        # This is a simplification; a full match would have more overs.
        self.max_overs = (20 // 6) + (1 if (20 % 6) > 0 else 0)


    def update(self, runs_this_ball, is_wicket_this_ball, total_score, total_wickets, total_balls_bowled_innings):
        self.score = total_score
        self.wickets = total_wickets

        if total_balls_bowled_innings >= 0: # Allow update for 0 balls (initial state)
            self.overs_done = total_balls_bowled_innings // 6
            self.balls_in_current_over = total_balls_bowled_innings % 6
        else:
            self.overs_done = 0
            self.balls_in_current_over = 0

        if is_wicket_this_ball:
            self.last_ball_outcome_display = "WICKET!"
        elif isinstance(runs_this_ball, int):
            self.last_ball_outcome_display = str(runs_this_ball)
        elif isinstance(runs_this_ball, str) and runs_this_ball == "wicket": # Handle if "wicket" string passed as runs
             self.last_ball_outcome_display = "WICKET!"
        else:
            self.last_ball_outcome_display = "-"
    ...
    def draw(self, surface):
        score_text = f"{self.batting_team_name}: {self.score} / {self.wickets}"
        # Format overs: X.Y / MaxOvers (e.g. 2.3 / 3 overs for a 20 ball match)
        overs_text = f"Overs: {self.overs_done}.{self.balls_in_current_over} / {self.max_overs}"
        last_ball_text = f"Last Ball: {self.last_ball_outcome_display}"

        score_surf = self.font.render(score_text, True, self.text_color)
        overs_surf = self.font.render(overs_text, True, self.text_color)
        last_ball_surf = self.font.render(last_ball_text, True, self.text_color)

        max_width = max(score_surf.get_width(), overs_surf.get_width(), last_ball_surf.get_width())

        total_height = 3 * self.line_height + 2 * self.padding
        bg_rect_width = max_width + 2 * self.padding

        bg_rect_x = self.screen_width - bg_rect_width - 10
        bg_rect_y = self.y_position

        scoreboard_bg_surf = pygame.Surface((bg_rect_width, total_height), pygame.SRCALPHA)
        scoreboard_bg_surf.fill(self.bg_color)
        surface.blit(scoreboard_bg_surf, (bg_rect_x, bg_rect_y))

        surface.blit(score_surf, (bg_rect_x + self.padding, bg_rect_y + self.padding))
        surface.blit(overs_surf, (bg_rect_x + self.padding, bg_rect_y + self.padding + self.line_height))
        surface.blit(last_ball_surf, (bg_rect_x + self.padding, bg_rect_y + self.padding + 2 * self.line_height))


class Team: # ... (No change)
    def __init__(self, name, color_tuple): self.name = name; self.color = color_tuple
class Bowler: # ... (No change)
    ...
    def update_action(self):
        if self.action_current_frame<self.action_total_frames:self.action_current_frame+=1;pd=self.action_total_frames//5;cp=self.action_current_frame//pd
...
class Batsman: # ... (No change)
    ...
    def start_action(self,outcome):self.action_current_frame=0
        ...
        elif outcome=="wicket":self.current_action="wicket_bowled"
    ...
    def update_animation(self):
        if self.action_current_frame>=self.action_frames_total:self.current_action="idle";self.bat_angle=0;return False
        self.action_current_frame+=1;p=self.action_current_frame/self.action_frames_total
        if self.current_action=="defensive":self.bat_angle=0
        elif self.current_action=="push":self.bat_angle=10*math.sin(p*math.pi)
        elif self.current_action=="drive":self.bat_angle=-30+60*math.sin(p*math.pi)
        elif self.current_action=="six_swing":self.bat_angle=-60+90*math.sin(p*math.pi)
        return True
...
class Fielder: # ... (No change)
    ...
    def update_movement(self):
        ...
        if self.action_frames>=self.max_action_frames:self.is_moving=False;self.reset_position();return False
        dx=self.target_x-self.x;dy=self.target_y-self.y;dist=math.hypot(dx,dy)
        if dist<self.speed:self.x,self.y=self.target_x,self.target_y;self.is_moving=False;return False
        self.x+=(dx/dist)*self.speed;self.y+=(dy/dist)*self.speed;return True
...
class Crowd: # ... (No change)
    ...
    def set_reaction(self,outcome):new_reaction_level=0
        if isinstance(outcome,str)and outcome=="wicket":new_reaction_level=2
        elif isinstance(outcome,int):
            if outcome==6 or outcome==4:new_reaction_level=2
            elif outcome>=1:new_reaction_level=1
            else:new_reaction_level=0
        if new_reaction_level>self.current_reaction_level:self.current_reaction_level=new_reaction_level;self.reaction_timer=self.reaction_duration_frames
        elif new_reaction_level==0 and self.current_reaction_level!=0:self.reaction_timer=FPS//3
        elif self.current_reaction_level==0 and new_reaction_level==0:self.reaction_timer=0
    def update(self):self.wave_time+=0.15
        if self.reaction_timer>0:self.reaction_timer-=1
        if self.reaction_timer==0:self.current_reaction_level=0
        for fan in self.fans:amplitude=0;eff_speed=fan['anim_speed_factor']
        if self.current_reaction_level==1:amplitude=fan['anim_amplitude_base']*1.5;eff_speed*=1.2
        elif self.current_reaction_level==2:amplitude=fan['anim_amplitude_base']*2.5;eff_speed*=1.5
        if amplitude>0:fan['y_offset']=math.sin(self.wave_time*eff_speed+fan['rect'].x*0.05)*amplitude
        else:fan['y_offset']=0
    def draw(self,s):
...
class CricketGame:
    def __init__(self):
        ...
        self.font = pygame.font.Font(None, 30) # Default font for scoreboard
        ...
        self.scoreboard = Scoreboard(self.font, SCREEN_WIDTH)
        self.current_score = 0
        self.current_wickets = 0
        self.total_balls_bowled_innings = 0
        ...
        if platform.system()=="Emscripten":
            try:
                ...
                if match_data_js:
                    match_data_py=match_data_js.to_py();print("Animation rcvd match_data:",match_data_py)
                    def hex_to_rgb(h):h=h.lstrip('#');return tuple(int(h[i:i+2],16)for i in(0,2,4))if len(h)==6 else(0,0,255)
                    ...
                    if not self.ball_log:self.ball_log=[0,1,4,"wicket",6,0,0,2,0,4,0,"wicket",0,1,0,6,0,0,2,0];print("Ext log empty, using default.")
                    if self.team_a and self.team_b and self.ball_log:
                        self.scoreboard.set_teams(self.team_a.name, self.team_b.name, self.team_a.name)
                        self.game_state="animation"; external_data_loaded=True;print(f"Starting anim for {self.team_a.name} vs {self.team_b.name}")
                    else:print("Err processing ext data")
            except Exception as e:print(f"Err access Pyodide data: {e}")

        self.ui=UI(self)

        if external_data_loaded:
            self.initialize_animation_elements()
        if not external_data_loaded: print("CricketGame Init, using internal UI.")

    def initialize_animation_elements(self):
        self.set_player_positions(init_setup=True);
        bowl_clr = self.team_b.color if self.team_b else BLUE;
        self.fielders=[]
        ...
        batting_team_name_for_sb = self.team_a.name if self.team_a else "Batting Team" # Fallback
        # Ensure teams are set for scoreboard if not already done by external data load path
        if self.team_a and self.team_b and not self.scoreboard.batting_team_name:
             self.scoreboard.set_teams(self.team_a.name, self.team_b.name, batting_team_name_for_sb)
        self.scoreboard.update(0, False, 0, 0, 0)


    def run(self):
        prev_st=self.game_state;
        is_direct_animation_mode = platform.system() == "Emscripten" and js.globals.get('match_data_for_animation') and self.bowler is not None

        for ev in pygame.event.get():
            if ev.type==pygame.QUIT:return False
            if not is_direct_animation_mode :
                 self.ui.handle_event(ev)

        if prev_st!="animation" and self.game_state=="animation":
            if self.team_a and self.team_b and self.ball_log:
                if not self.bowler : self.initialize_animation_elements()
            elif not is_direct_animation_mode:
                print("Error: Anim start prereqs not met.");self.game_state="log_input";self.ui.log_message_surf=self.font.render("Select teams and log first!",True,RED)

        self.screen.fill(SKY_BLUE)
        if self.game_state == "animation":
            if self.crowd : self.crowd.update()
        ...
        if self.game_state=="menu":self.ui.draw_initial_screen()
            ...
            if not self.bowler:
                self.ui.draw_animation_scene(None, None, [], None, self.crowd, "Error: Match data missing.", False);
            else:
                ...
                if self.animation_phase==ANIMATION_PHASE_PRE_BALL:
                    ...
                    if self.bowler.update_action():
                        ...
                        if self.animation_phase!=ANIMATION_PHASE_BALL_TRAVEL: self.animation_phase=ANIMATION_PHASE_SHOWING_OUTCOME
                    ...
                    if not self.ball.update_trajectory():
                        ...
                        if current_outcome == "wicket":
                            self.batsman_stumps_hit = True; self.ball.x = self.batsman.x; self.ball.y = STUMPS_LINE_Y_BATSMAN + STUMP_HEIGHT/2; self.ball.visible = False
                    ...
                    if not self.batsman.update_animation():
                        current_outcome = self.ball_log[self.current_ball_index]
                        self.total_balls_bowled_innings += 1
                        ...
                        if isinstance(current_outcome, int) and current_outcome >= 0 and not is_wicket_this_ball:
                            ...
                            for i in fielders_to_move_indices:
                                fielder = self.fielders[i]
                                target_x = fielder.original_x + random.randint(-25, 25); target_y = fielder.original_y + random.randint(-25, 25)
                                target_x = max(FIELDER_RADIUS, min(target_x, SCREEN_WIDTH - FIELDER_RADIUS)); target_y = max(int(SCREEN_HEIGHT*0.20)+FIELDER_RADIUS, min(target_y, SCREEN_HEIGHT - FIELDER_RADIUS - 30))
                                fielder.start_move(target_x, target_y)
                            self.fielding_phase_timer = FIELDER_MAX_MOVE_FRAMES
                            self.animation_phase = ANIMATION_PHASE_FIELDING
                        else:
                            self.animation_phase = ANIMATION_PHASE_SHOWING_OUTCOME

                elif self.animation_phase == ANIMATION_PHASE_FIELDING:
                    ...
                    if not still_fielders_moving or self.fielding_phase_timer <=0:
                        for fielder in self.fielders: fielder.reset_position()
                        # Crowd reaction was already set before fielding phase if it was a scoring shot
                        self.animation_phase = ANIMATION_PHASE_SHOWING_OUTCOME
        ...
        elif self.game_state=="menu":self.ui.draw_initial_screen() # Ensure other states are drawn if not animation
        elif self.game_state=="team_selection":self.ui.draw_team_selection_screen()
        elif self.game_state=="log_input":self.ui.draw_log_input_screen()

        pygame.display.flip();self.clock.tick(FPS);return True

# --- UI Class --- (Full definition to be included)
class UI:
    ...
    def draw_stumps(self,sfc,xc,yb,hit=False):st_px=[xc-STUMPS_GAP-STUMP_WIDTH//2,xc-STUMP_WIDTH//2,xc+STUMPS_GAP-STUMP_WIDTH//2];clr=STUMP_HIT_COLOR if hit else STUMP_COLOR;
        for sx in st_px:
            if hit and random.choice([True,False]):pygame.draw.line(sfc,clr,(sx,yb),(sx+random.randint(-5,5),yb-STUMP_HEIGHT-random.randint(0,10)),STUMP_WIDTH+1)
    ...
    def draw_animation_scene(self, bowler, batsman, fielders, ball_obj, crowd_obj, current_outcome_text="", batsman_stumps_hit=False):
        ...
        if crowd_obj: crowd_obj.draw(self.game.screen)
